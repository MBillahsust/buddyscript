  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/posts", {
        ...
        body: JSON.stringify({ content: content.trim(), imageUrl, visibility: "PUBLIC" }),
      })
    ...
    <div className="_feed_inner_text_area _b_radious6 _padd_b24 _padd_t24 _padd_r24 _padd_l24 _mar_b16">
      ...
      {imagePreview && (
        <div style={{ position: "relative", marginTop: "12px", display: "inline-block" }}>
          <img src={imagePreview} alt="Preview" style={{ maxHeight: 200, borderRadius: 6, maxWidth: "100%" }} />
          <button
            type="button"
            onClick={() => { setImageFile(null); setImagePreview(null); if (fileRef.current) fileRef.current.value = "" }}
            style={{ position: "absolute", top: 4, right: 4, background: "rgba(0,0,0,0.6)", color: "#fff", border: "none", borderRadius: "50%", width: 24, height: 24, cursor: "pointer", lineHeight: "24px", textAlign: "center" }}
          >
            ×
          </button>
        </div>
